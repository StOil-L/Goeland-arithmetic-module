...
func TestSimplexeRatint2() {
	fmt.Println(" -------- TEST 31 -------- ")
	fmt.Println(" X < 3 et X < 2 - X int, Y int")
	x := types.MakerMeta("X", -1)
	trois := types.MakerConst(types.MakerId("3"), tInt)
	deux := types.MakerConst(types.MakerId("2"), tInt)
	p1 := types.MakePred(types.MakerId("less"), []types.Term{x, trois}, typing.MkTypeArrow(typing.MkTypeCross(tInt, tInt), tProp))
	p2 := types.MakePred(types.MakerId("less"), []types.Term{x, deux}, typing.MkTypeArrow(typing.MkTypeCross(tInt, tInt), tProp))
	systeme := []types.Form{p1, p2}
	found, solution := ari.ApplySimplexRule(systeme)
	fmt.Printf("Solution trouvée : %v = %v \n", found, solution.ToString())
}
